...
const Profile = () => {
  ...
  const profileMenu = (
    <div className="bg-white dark:bg-gray-800 p-4 rounded-md shadow-md">
      <Button type="default" block className="mt-4 mb-2" onClick={() => history(`/user-profile/${user?.firstName}`)}>Profile</Button>
      <Button type="default" block className="mb-2" onClick={() => history(`/user-payments/${user?.firstName}`)}>Payments</Button>
      <Button type="default" block onClick={logoutHandler}>Logout</Button>
    </div>
  );
        ...
        <div className="hidden md:flex gap-4">
          ...
          <Dropdown overlay={profileMenu} trigger={["click"]} placement="bottomRight">
            <div className="relative cursor-pointer">
              <Avatar className="bg-blue-500" size={40} src={user?.profilePhoto || undefined}>
        ...
        <Card className="w-full sm:max-w-md lg:max-w-lg shadow-lg rounded-lg bg-white dark:bg-gray-800 p-8">
          <div className="text-center mb-6">
            ...
            <div className="relative inline-block mb-4">
              <Avatar size={110} className="bg-blue-500" src={user?.profilePhoto || undefined}>
                {!user?.profilePhoto && user?.firstName?.charAt(0).toUpperCase()}
              </Avatar>
            ...
            <p className="text-sm text-gray-500 mt-1">Student ID: <strong>{user?.studentId}</strong></p>
            <p className="text-sm mt-1">
              Status:{" "}
              <span className={user?.membership?.status === "active" ? "text-green-600 font-semibold" : "text-yellow-600 font-semibold"}>
          ...
          <div className="space-y-4">
            {user?.membership?.status === "active" && (
              <Button type="primary" className="w-full" onClick={() => history(`/user-classes/${user?.firstName}`)}>
                View My Classes
              </Button>
            )}
            {user?.membership?.status !== "active" && (
              <Button type="default" className="w-full" onClick={() => history(`/user-enroll/${user?.firstName}`)}>
                Enroll / Pay
              </Button>
            )}
            ...
            {editing ? (
              <Form form={form} onFinish={handleSave} layout="vertical">
                ...
                <Form.Item label="Phone" name="telephoneNumber" rules={[{ required: true }]}>
                  <Input />
                </Form.Item>
                <div className="flex gap-2">
                  <Button type="primary" htmlType="submit" block disabled={loading}>
                    {loading ? <Spin indicator={<LoadingOutlined />} /> : "Save Changes"}
                  </Button>
              ...
              <div className="space-y-2">
                ...
                <Button type="primary" danger className="w-full" onClick={handleDeleteAccount}>
                  Delete Account
                </Button>
              </div>
            )}
